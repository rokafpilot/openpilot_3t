...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    # longitudinal
    ret.longitudinalTuning.kpBP = [0., 10.*CV.KPH_TO_MS, 20.*CV.KPH_TO_MS, 50.*CV.KPH_TO_MS, 80.*CV.KPH_TO_MS, 130.*CV.KPH_TO_MS]
    ret.longitudinalTuning.kpV = [1.2, 0.94, 0.8, 0.6, 0.5, 0.4]
    ret.longitudinalTuning.kiBP = [0., 30. * CV.KPH_TO_MS , 50. * CV.KPH_TO_MS, 100.*CV.KPH_TO_MS, 130. * CV.KPH_TO_MS]
    ret.longitudinalTuning.kiV = [0.025, 0.04, 0.055, 0.04, 0.025]
    ...
    ret.startAccel = -0.5
    ret.stopAccel = -2.0
    ret.startingAccelRate = 6.5  # brake_travel/s while releasing on restart
