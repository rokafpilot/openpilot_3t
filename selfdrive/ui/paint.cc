...
static void ui_draw_vision_lane_lines(UIState *s) {
  ...
  if (!scene.end_to_end) {
    ...
    track_bg = nvgLinearGradient(s->vg, s->fb_w, s->fb_h, s->fb_w, s->fb_h * .4,
                                          COLOR_MAGENTA_ALPHA(200), COLOR_MAGENTA_ALPHA(0));
  } else {
    track_bg = nvgLinearGradient(s->vg, s->fb_w, s->fb_h, s->fb_w, s->fb_h * .4,
...
{  
  ...
  if ((s->scene.currentGear < 9) && (s->scene.currentGear !=0)) {
    ui_draw_text(s, center_x, center_y, strGear, 27 * 10., COLOR_CYAN, "sans-bold");
  } else if (s->scene.currentGear == 14 ) {
    ui_draw_text(s, center_x, center_y, "R", 27 * 10., COLOR_RED, "sans-bold");
  } else if (ngetGearShifter == 1 ) {
    ui_draw_text(s, center_x, center_y, "P", 27 * 10., COLOR_BLUE, "sans-bold");
  } else if (ngetGearShifter == 3 ) {
    ui_draw_text(s, center_x, center_y, "N", 27 * 10., COLOR_WHITE, "sans-semibold");
  } else {
    ui_draw_text(s, center_x, center_y, "", 27 * 10., COLOR_GREEN, "sans-bold");
  }
}
...
static void ui_draw_vision_scc_gap(UIState *s) {
  ...
  if(gap <= 0) {
    ...
    textColor = nvgRGBA(0, 255, 255, 200);
    textSize = 38.f;
  }
...
static void ui_draw_vision_maxspeed(UIState *s) {
  ...
  if (is_cruise_set) {
    ...
    ui_draw_text(s, text_x, 100 + y, str, 33 * 2.5, COLOR_CYAN, "sans-semibold");
    ...
    ui_draw_text(s, text_x, 195 + y, str, 48 * 2.5, COLOR_MAGENTA, "sans-bold");
  } else {
    if(longControl)
        ui_draw_text(s, text_x , 100 + y, "OP", 25 * 3, COLOR_WHITE_ALPHA(150), "sans-bold");
    else
        ui_draw_text(s, text_x, 100 + y, "MAX", 25 * 3, COLOR_WHITE_ALPHA(150), "sans-semibold");

    ui_draw_text(s, text_x, 195 + y, "N/A", 42 * 2.5, COLOR_YELLOW_ALPHA(150), "sans-bold");
  }
}
